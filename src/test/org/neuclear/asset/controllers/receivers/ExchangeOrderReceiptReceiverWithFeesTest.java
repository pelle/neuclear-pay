...
public class ExchangeOrderReceiptReceiverWithFeesTest extends AbstractExchangeReceiverTest {
    ...
    public void testExchangeOrderReceipt() throws NeuClearException, InvalidTransferException, LowlevelLedgerException, UnknownBookException, InvalidTransactionException {
        ...
        ExchangeOrderReceipt receipt = (ExchangeOrderReceipt) new ExchangeOrderReceiver(signer, ledger).receive(new ExchangeOrderBuilder(asset, agent, new Amount(amount), new Date(System.currentTimeMillis() + 10000), new BidItem[]{new BidItem(shoes, new Amount(amount))}, "test").convert("bob", signer));
