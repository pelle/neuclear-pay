 ...
 * Time: 1:38:45 PM
 */
public class TransferReceiptReceiverWithFeesTest extends AbstractSigningTest {

    public TransferReceiptReceiverWithFeesTest(String string) throws NeuClearException, GeneralSecurityException {
        super(string);
    ...
    public void testTransferReceipt() throws NeuClearException, InvalidTransferException, LowlevelLedgerException, UnknownBookException, InvalidTransactionException {
        ...
        receiver.receive(new TransferReceiptBuilder((TransferOrder) new TransferOrderBuilder(asset, recipient, new Amount(amount), "test").convert("bob", signer), new Date()).convert(signeralias, signer));
        double fee = asset.getFeeStructure().calculateFee(amount);
        assertEquals(aliceBalance + 10 - fee, ledger.getBalance(asset.getServiceId(), recipient.getName()), 0);
