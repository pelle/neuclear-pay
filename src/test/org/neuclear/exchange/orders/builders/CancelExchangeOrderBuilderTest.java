...
$Id: CancelExchangeOrderBuilderTest.java,v 1.1 2004/04/05 16:31:45 pelle Exp $
$Log: CancelExchangeOrderBuilderTest.java,v $
 ...
 * Time: 9:11:44 PM
 */
public class CancelExchangeOrderBuilderTest extends AbstractExchangeOrderTest {
    public CancelExchangeOrderBuilderTest(String string) throws NeuClearException, GeneralSecurityException {
        super(string);
    }

    protected void verifyObject(SignedNamedObject obj) throws NonExistingSignerException {
        assertNotNull(obj);
        assertTrue(obj instanceof CancelExchangeOrder);
        CancelExchangeOrder cancel = (CancelExchangeOrder) obj;
        assertEquals(getSigner().getPublicKey("neu://test").getEncoded(), cancel.getSignatory().getPublicKey().getEncoded());
...
//        assertEquals(getBob().getPublicKey().getEncoded(), order.getRecipient().getSignatory().getPublicKey().getEncoded());
    ...
    protected Class getRequiredClass() {
        return CancelExchangeOrder.class;
    }
    ...
    protected Builder createBuilder() throws NeuClearException, InvalidTransferException, XMLException {
        ...
        CancelExchangeOrderBuilder builder = new CancelExchangeOrderBuilder((ExchangeOrderReceipt) rb.convert("neu://test/bux", getSigner()));
        return builder;
    }
