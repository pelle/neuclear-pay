...
/*
$Id: TransferOrderReceiver.java,v 1.1 2004/07/21 23:11:22 pelle Exp $
    ...
    public SignedNamedObject receive(SignedNamedObject obj) throws UnsupportedTransaction, NeuClearException {
        try {
            ...
            if (!signer.canSignFor(name)) {
                ...
                final PostedTransaction posted = ledger.verifiedTransfer(order.getAsset().getSignatory().getName(), order.getDigest(), order.getSignatory().getName(), order.getRecipient(), order.getAmount().getAmount(), order.getComment());
